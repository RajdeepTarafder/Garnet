# -*- coding: utf-8 -*-
"""
Created on Thu Nov  3 15:23:21 2022

@author: user
"""


data_outsample= pd.read_excel('Garnet_filtered_F.xlsx',sheet_name='Unlabelled')
data_rev1=data_outsample[['SiO2','TiO2','Al2O3','Cr2O3','FeOtotal','MnO','MgO','CaO']]
#data_outsample= data_outsample.drop(columns=['author','journal','region','sample name','host-rock_type/metamorphic_facies','lithology/protolith','P(kbar)','T(°C)',])



data_rev1= data_rev1.replace('<0.03', 0)
data_rev1= data_rev1.replace(np.nan, 0)

data_rev1 = data_rev1.applymap(lambda x: 0 if(x < 0) else x)



data_rev1.SiO2=data_rev1.SiO2.map(float)
data_rev1.TiO2=data_rev1.TiO2.map(float)
data_rev1.Al2O3=data_rev1.Al2O3.map(float)
data_rev1.Cr2O3=data_rev1.Cr2O3.map(float)
data_rev1.FeOtotal=data_rev1.FeOtotal.map(float)
data_rev1.MnO=data_rev1.MnO.map(float)
data_rev1.MgO=data_rev1.MgO.map(float)
data_rev1.CaO=data_rev1.CaO.map(float)

#SiO conversion
df_SiO =data_rev1.copy()
#taking out  df/SiOO2)
...
df_SiO = df_SiO.applymap(lambda x: math.log10(x) if((x!=0)) else 0)


#CaO converSiOon
df_CaO=data_rev1.copy()
#taking out  df/CaO)
...
data_2= pd.concat([df_SiO,df_CaO,df_MgO,df_MnO,df_Al2O3,df_Cr2O3,df_FeO,df_Si,df_Ca,df_Mg,df_Mn,df_Al,df_Cr,df_Fe,df_n['Ca/(Ca+Mg)'],df_n['Mg/(Fe+Mg)']],axis=1)
...
predictions_2 = xgb_clf.predict(data_2 )
