# -*- coding: utf-8 -*-
"""
Created on Sun Nov  6 14:03:37 2022
...
reduced_data = sc.transform(X_train)
...
for i in range(n):
    plt.arrow(0, 0, reduced_data[i, 0], reduced_data[i, 1],
              color='k', alpha=0.7, linewidth=1, )
    plt.text(reduced_data[i, 0]*1.01, reduced_data[i, 1]*1.01, reduced_data.index[i],
             ha='center', va='center', color='k', fontsize=12)

plt.xlabel(f"{reduced_data.columns[0]}")
plt.ylabel(f"{reduced_data.columns[1]}")
